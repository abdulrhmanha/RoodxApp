...
export default function StoryViewer({ user, onClose }) {
  ...
  const onVideoLoad = (meta) => {
    startProgress(meta.duration * 1000);
  };
    ...
    >
      <View style={styles.container}>
        {current.type === 'image' ? (
          ...
          <Video source={{ uri: current.url }} style={styles.storyMedia} resizeMode="cover" onEnd={onVideoEnd} onLoad={onVideoLoad} paused={false} repeat={false} />
        )}
